...
import { LayoutDashboard, Table2, PlusCircle, Upload, Search, AlertTriangle, CheckCircle2, Trash2, Download, Plane, Globe2 } from 'lucide-react';
...
const flightFields = ['country','bookingDate','category','employeeId','employeeName','pil','entity','caseManager','passengerName','relationship','departureDate1','departureDate2','departure1','departure2','airfare','trxFee','card','remark','iaccDetail','costCenter','status'];
const flightLabels = { country:'Country', bookingDate:'Booking Date', category:'Category', employeeId:'EE ID', employeeName:'Employee Name', pil:'PIL', entity:'Entity', caseManager:'Case Manager', passengerName:'Passenger Name', relationship:'Relationship', departureDate1:'Departure Date 1', departureDate2:'Departure Date 2', departure1:'Departure 1', departure2:'Departure 2', airfare:'Airfare', trxFee:'TRX Fee', card:'CARD', remark:'Remark', iaccDetail:'iACC Detail', costCenter:'Cost Center', status:'Status' };
...
function inferCountryFromRoute(code){ const c = String(code || '').toUpperCase(); const map = { ICN:'Korea', GMP:'Korea', PUS:'Korea', BLR:'India', TRV:'India', BOM:'India', DEL:'India', IAD:'USA', HNL:'USA', SEA:'USA', LAX:'USA', JFK:'USA', SYD:'Australia', MEL:'Australia', SIN:'Singapore', TPE:'Taiwan', TYO:'Japan', NRT:'Japan', HND:'Japan', HKG:'Hong Kong' }; return map[c] || 'Unknown'; }
function flightCountry(r){ return r.country || inferCountryFromRoute(r.departure2) || inferCountryFromRoute(r.departure1) || 'Unknown'; }
...
function normalizeFlight(raw){ const departure2 = getRaw(raw,'Departure 2') || ''; return { id:crypto.randomUUID(), country:getRaw(raw,'Country') || inferCountryFromRoute(departure2), bookingDate:makeExcelDate(getRaw(raw,'Booking Date','Booking Dat')), category:getRaw(raw,'Category') || '', employeeId:getRaw(raw,'EE ID','Employee ID','Employee') || '', employeeName:getRaw(raw,'Employee Name') || '', pil:getRaw(raw,'Pil','PIL') || '', entity:getRaw(raw,'Entity') || '', caseManager:getRaw(raw,'Case Manager') || '', passengerName:getRaw(raw,'Passenger Name') || '', relationship:getRaw(raw,'Relationship') || '', departureDate1:makeExcelDate(getRaw(raw,'Departure Date 1','Departure Date')), departureDate2:makeExcelDate(getRaw(raw,'Departure Date 2')), departure1:getRaw(raw,'Departure 1','Departure') || '', departure2, airfare:num(getRaw(raw,'Airfare')), trxFee:num(getRaw(raw,'TRX Fee','TRX fee')), card:getRaw(raw,'CARD','Card') || '', remark:getRaw(raw,'Remark') || '', iaccDetail:getRaw(raw,'iACC Detail','iAC Detail','IACC Detail') || '', costCenter:getRaw(raw,'Cost Center') || '', status:getRaw(raw,'Status') || 'BOOKED' }; }
...
function App(){
  ...
  const [view, setView] = useState('overviewDashboard');
  ...
  const overviewStats = useMemo(()=>{
    const tuitionAnnual = payments.reduce((s,r)=>s+num(r.amount),0);
    const flightAnnual = flights.reduce((s,r)=>s+num(r.airfare)+num(r.trxFee),0);
    const countries = [...new Set([...payments.map(r=>r.country || 'Unknown'), ...flights.map(flightCountry)])];
    const byCountry = countries.map(country => {
      const tuition = payments.filter(r=>(r.country || 'Unknown')===country).reduce((s,r)=>s+num(r.amount),0);
      const flight = flights.filter(r=>flightCountry(r)===country).reduce((s,r)=>s+num(r.airfare)+num(r.trxFee),0);
      const tuitionCount = payments.filter(r=>(r.country || 'Unknown')===country).length;
      const flightCount = flights.filter(r=>flightCountry(r)===country).length;
      return { country, tuition, flight, tuitionCount, flightCount, totalCount: tuitionCount + flightCount };
    }).sort((a,b)=>(b.tuition+b.flight)-(a.tuition+a.flight));
    const byMonth = months.map(month => {
      const tuition = payments.filter(r=>monthIndex(r.invoiceMonth)===monthIndex(month)).reduce((s,r)=>s+num(r.amount),0);
      const flight = flights.filter(r=>monthFromDate(r.bookingDate)===month).reduce((s,r)=>s+num(r.airfare)+num(r.trxFee),0);
      return { month, Tuition:tuition, Flight:flight, Total:tuition+flight };
    });
    const topMonth = byMonth.reduce((max,m)=>m.Total > max.Total ? m : max, { month:'-', Total:0 });
    return { tuitionAnnual, flightAnnual, grandTotal:tuitionAnnual+flightAnnual, byCountry, byMonth, topMonth, totalRecords:payments.length+flights.length };
  },[payments, flights]);

  const isFlight = view.startsWith('flight');
  const isOverview = view === 'overviewDashboard';
  const titleMap = { overviewDashboard:'통합 비용 대시보드', paymentDashboard:'Sirva Payment Dashboard', paymentTable:'Payment Tracker', paymentForm:'Payment 개별 등록', paymentUpload:'Payment Excel Upload', flightDashboard:'Flight Booking Dashboard', flightTable:'항공권 데이터 관리', flightForm:'항공권 개별 등록', flightUpload:'항공권 Excel Upload' };
  ...
  function exportExcel(){
    if(isOverview){
      const ws = XLSX.utils.json_to_sheet(overviewStats.byCountry.map(r=>({ Country:r.country, 'Tuition Cost':r.tuition, 'Flight Cost':r.flight, 'Tuition Count':r.tuitionCount, 'Flight Count':r.flightCount, 'Total Count':r.totalCount })));
      const wb = XLSX.utils.book_new(); XLSX.utils.book_append_sheet(wb, ws, 'Integrated Dashboard'); XLSX.writeFile(wb, 'integrated-cost-dashboard.xlsx'); return;
    }
  ...
  return <div className="app">
    <aside className="side"><h1>Global<br/>Mobility</h1>
      <button onClick={()=>setView('overviewDashboard')} className={view==='overviewDashboard'?'active':''}><Globe2/> 통합 대시보드</button>
      <div className="sideGroup">Tuition / Payment</div>
    ...
    <main>
      <header><div><p>{isOverview ? 'Tuition + Flight / Integrated Cost View' : isFlight ? 'Flight Booking / Airfare Cost' : 'Global Mobility Tuition / Education Cost'}</p><h2>{titleMap[view]}</h2></div><button className="ghost" onClick={exportExcel}><Download size={16}/> Export Excel</button></header>

      {view==='overviewDashboard' && <section className="dash"><div className="hero four"><div><span>Annual Usage Cost</span><strong>{overviewStats.grandTotal.toLocaleString()}</strong><small>학비+항공권 입력금액 합산</small></div><div className="split"><b>Tuition / Education</b><strong>${overviewStats.tuitionAnnual.toLocaleString()}</strong><small>Payment Tracker</small></div><div className="split orange"><b>Flight Booking</b><strong>₩{overviewStats.flightAnnual.toLocaleString()}</strong><small>Airfare + TRX Fee</small></div><div className="split purple"><b>Monthly Peak</b><strong>{overviewStats.topMonth.month}</strong><small>{overviewStats.topMonth.Total.toLocaleString()} used</small></div></div><div className="panel wide"><h3>Monthly Usage Cost - Tuition vs Flight</h3><ResponsiveContainer height={300}><BarChart data={overviewStats.byMonth}><XAxis dataKey="month"/><YAxis/><Tooltip/><Legend/><Bar dataKey="Tuition" stackId="a" fill="#22d3ee"/><Bar dataKey="Flight" stackId="a" fill="#fb923c"/></BarChart></ResponsiveContainer></div><div className="panel wide"><h3>Country Usage Cost - Tuition & Flight</h3><ResponsiveContainer height={320}><BarChart data={overviewStats.byCountry}><XAxis dataKey="country"/><YAxis/><Tooltip/><Legend/><Bar dataKey="tuition" name="Tuition" fill="#22d3ee"/><Bar dataKey="flight" name="Flight" fill="#fb923c"/></BarChart></ResponsiveContainer></div><div className="panel"><h3>Country Cost Detail</h3><div className="tableWrap short"><table><thead><tr><th>Country</th><th>Tuition Cost</th><th>Flight Cost</th><th>Tuition Count</th><th>Flight Count</th><th>Total Count</th></tr></thead><tbody>{overviewStats.byCountry.map(c=><tr key={c.country}><td>{c.country}</td><td>${c.tuition.toLocaleString()}</td><td>₩{c.flight.toLocaleString()}</td><td>{c.tuitionCount}</td><td>{c.flightCount}</td><td>{c.totalCount}</td></tr>)}</tbody></table></div></div><div className="panel"><h3>Monthly Cost Detail</h3><table><thead><tr><th>Month</th><th>Tuition</th><th>Flight</th><th>Total</th></tr></thead><tbody>{overviewStats.byMonth.map(m=><tr key={m.month}><td>{m.month}</td><td>${m.Tuition.toLocaleString()}</td><td>₩{m.Flight.toLocaleString()}</td><td>{m.Total.toLocaleString()}</td></tr>)}</tbody></table></div></section>}
      ...
      {view==='flightTable' && <DataTable rows={filteredFlights} allRows={flights} setRows={setFlightSave} fields={flightFields} labels={flightLabels} query={query} setQuery={setQuery} reset={()=>setFlightSave(sampleFlights)} placeholder="국가, 직원명, 탑승객, 항공구간, Cost Center 검색" moneyFields={['airfare','trxFee']} />}
